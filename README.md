...
| `articles/<slug>.dark.tex` + `<slug>.dark.pdf` | Тёмный вариант (собирается из основного файла с опцией `[dark]`); читалка на сайте грузит его в тёмной теме. |
| `articles/zhpl.sty` | Пакет журнала: преамбула, окружения теорем, макросы титульного блока; опция `[dark]`. |
...
1. `cp articles/template.tex articles/<slug>.tex`, заполнить TODO по руководству §2.
   Создать обёртку `articles/<slug>.dark.tex` (копия существующей, поменять имя в `\input`).
2. `cd articles && latexmk` — собрать оба PDF. Закоммитить `.tex`, `.dark.tex`, `.pdf`, `.dark.pdf`.
