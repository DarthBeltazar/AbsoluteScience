...
- `articles/articles.json` — **single source of truth** for article metadata and for the
  `people` roster (id + full name) shown on the site. An entry with `"hidden": true` is skipped
  by the generator (excluded from the `#issue` teasers, the article-count stat, per-author
  counts, and `sitemap.xml`) while its `.tex`/`.pdf`/`.html` stay in the repo and reachable by
  direct URL (its generated page gets `<meta name="robots" content="noindex">`) — use this to
...
- `scripts/gen_site.py` — regenerates, from `articles.json`: the `#issue` teaser cards and stat
  ...
  regions; a full `articles/<slug>.html` page per article (including hidden ones); and
  `sitemap.xml`. `--check` fails if any of these is out of date.
- `index.html` — static journal site (single page, no framework); the full text of each article
  ...
  `articles.json`, not these regions.
...
2. Add a `<slug>` object to `articles/articles.json` (id, issue, kicker, titleHtml, authors,
   abstract, highlights, keywords, citeHtml, doi). If a new author appears, also add them to the
   top-level `people` array (id + full name) — that's what per-author article counts are computed
   from (exact-substring match against each article's `authors` string).
3. Run `python scripts/gen_site.py` (or `make site`) — it rebuilds the `#issue` teasers, the new
   `articles/<slug>.html` page, and every counter (including `#people` article counts). Never
   hand-edit the generated regions of `index.html` or an `articles/<slug>.html` file.
