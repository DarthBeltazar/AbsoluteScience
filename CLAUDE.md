...
- `assets/theme.js` — shared light/dark toggle logic (button in the masthead), used by both
  `index.html` and `404.html`. Persists the explicit choice to `localStorage` (`zhpl:theme`);
  with no explicit choice, follows `prefers-color-scheme` and reacts to it changing live.
  Exposes `window.ZhplTheme.{siteIsDark,storedTheme,applyTheme}` — `index.html`'s own inline
  script reads `siteIsDark()` so the PDF reader's dark toggle defaults to the site's theme.
